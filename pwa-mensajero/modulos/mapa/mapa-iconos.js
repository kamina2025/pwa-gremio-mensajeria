 ...
 * @param {string} params.estado - 'creacion'|'asignado'|'en-camino'|'en-punto'|'entregado'|'no-entregado'
 * @param {number|string} params.secuencia - Número ordinal de la parada (ej: 1, 2, 3)
 * @param {string} [params.causal] - Texto abreviado de la causal en caso de fallo (ej: 'AUSENTE')
...
export function crearIconoParadaCyberpunkSVG({ estado = 'asignado', secuencia = 1, causal = '' }) {
    const config = ESTADOS_PARADA[estado] || ESTADOS_PARADA['asignado'];
    const colorFill = config.color;
    const colorGlow = config.glow;
    const seqStr = String(secuencia).padStart(2, '0');
    
    // Si hay causal en fallo, recortamos texto
    const txtCausal = estado === 'no-entregado' && causal ? causal.toUpperCase().substring(0, 7) : null;
    ...
    <svg xmlns="http://www.w3.org/2000/svg" width="48" height="64" viewBox="0 0 48 64">
        ...
        <g filter="url(#neon-glow)">
            <!-- Badge de Causal superior (Solo si es No-Entregado) -->
            ${txtCausal ? `
                <polygon points="4,2 44,2 40,12 8,12" fill="#ff3333" />
                <text x="24" y="9.5" fill="#000000" font-family="'Fira Code', monospace" font-weight="900" font-size="7" text-anchor="middle">${txtCausal}</text>
            ` : ''}
            ...
            <text x="17" y="32" fill="#ffffff" font-family="'Fira Code', monospace" font-weight="bold" font-size="12" text-anchor="middle">${seqStr}</text>
            ...
            <text x="33" y="31.5" fill="${colorFill}" font-size="9" text-anchor="middle">${config.symbol}</text>
    ...
    return {
        ...
        anchor: new google.maps.Point(24, 58) // La punta de la aguja coincide con la coordenada GPS exacta
    };
}

/**
 * Mantiene compatibilidad previa con la función Radar SVG si es requerida por el mapa de fondo
 */
